...
document.getElementById("img-c1").addEventListener("click", function () {
  ...
  window.location.href = `/tvshow/${document.getElementById("img-c1").dataset.img}`;
  console.log(window.location.href.split("/"));
  fetch(
    ` https://api.themoviedb.org/3/find/{external_id}?${api}&language=en-US&external_source=imdb_id`
  );
});
